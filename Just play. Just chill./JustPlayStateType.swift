...
protocol JustPlayWebLoadStateRepresentable {
    var type: JustPlayWebLoadState.StateType { get }
...
struct JustPlayWebLoadState: Equatable, JustPlayWebLoadStateRepresentable {
    ...
    static func idle() -> JustPlayWebLoadState {
        JustPlayWebLoadState(type: .idle, percent: nil, error: nil)
    }
    
    /// Создание состояния прогресса
    static func progress(_ percent: Double) -> JustPlayWebLoadState {
        JustPlayWebLoadState(type: .progress, percent: percent, error: nil)
    }
    
    /// Создание состояния успеха
    static func success() -> JustPlayWebLoadState {
        JustPlayWebLoadState(type: .success, percent: nil, error: nil)
    }
    
    /// Создание состояния ошибки
    static func error(_ err: String) -> JustPlayWebLoadState {
        JustPlayWebLoadState(type: .error, percent: nil, error: err)
    }
    
    /// Создание состояния отсутствия подключения
    static func offline() -> JustPlayWebLoadState {
        JustPlayWebLoadState(type: .offline, percent: nil, error: nil)
    }
    
    // MARK: - Методы сравнения
    
    /// Пользовательская реализация сравнения
    func isEqual(to other: JustPlayWebLoadState) -> Bool {
    ...
    static func == (lhs: JustPlayWebLoadState, rhs: JustPlayWebLoadState) -> Bool {
        lhs.isEqual(to: rhs)
    }
}
...
extension JustPlayWebLoadState {
...
extension JustPlayWebLoadState: CustomStringConvertible {
