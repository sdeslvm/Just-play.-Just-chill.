...
protocol JustPlayGradientProviding {
...
final class JustPlayGradientContainerView: UIView, JustPlayGradientProviding {
...
struct JustPlayWebViewBox: UIViewRepresentable {
    // MARK: - Свойства
    
    @ObservedObject var loader: JustPlayWebLoader
    
    // MARK: - Координатор
    
    func makeCoordinator() -> JustPlayWebCoordinator {
        JustPlayWebCoordinator { [weak loader] status in
    ...
    private func setupContainerView(with webView: WKWebView) {
        let containerView = JustPlayGradientContainerView()
